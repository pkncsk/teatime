...
def filter_e2(e_table,
    ...
    #test second pass
        ...
        if second_pass_tbl.shape[0] > 1:
            ...
            spliced_maf_age=spliced_maf_full.merge(external_data_table[['meta_name','ungapped_length','divergenc']], how='left',on='meta_name')
...
def filter_e_for_age(subfamily, 
    ...
    if (os.path.isfile(output_filepath) == False):
        ...
        for e_val_table in e_val_table_by_id:
            test= filter_e2(e_val_table,
                                   internal_id_dir, 
                                   maf_dir,
                                   subfamily,
                                   additional_evalue_cutoff,
...
if __name__ == '__main__' and 'get_ipython' in globals():
    # Skip this block if in terminal mode

    # Hardcoded test inputs for debugging
    test_internal_id_dir = '/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/dev/teatime/pipeline'
    test_e_table_dir = '/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/dev/teatime/pipeline'
    test_repeatmasker_filepath = '/rds/project/rds-XrHDlpCeVDg/users/pakkanan/data/resource/repeatmasker_table/hg38_repeatlib2014/hg38.fa.out.tsv'
    test_maf_dir = '/rds/project/rds-XrHDlpCeVDg/users/pakkanan/data/resource/multi_species_multiple_alignment_maf/cactus447/'
    test_external_data_table_filepath = '/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/data/resource/zoonomia_divergence_ref_table/species447_info_c.txt'
    test_output_dir = '/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/dev/teatime/pipeline'

    # Use one small known subfamily for quick debugging
    test_subfamily_list = ['THE1C']

    test=main(
        internal_id_dir=test_internal_id_dir,
        e_table_dir=test_e_table_dir,
        subfamily_list=test_subfamily_list,
        repeatmasker_filepath=test_repeatmasker_filepath,
        maf_dir=test_maf_dir,
        external_data_table_filepath=test_external_data_table_filepath,
        age_table_dir=test_output_dir,
        additional_evalue_cutoff=None,  # or some float like 1e-10
        target_species='hg38',
