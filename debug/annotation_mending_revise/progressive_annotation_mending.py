...
for block_id in block_id_list:
    block_df = multis_table[multis_table['tag']==block_id]
...
def convert_tag_to_internal_id(tag: str, subfamily: str) -> str:
    ...
    if tag.endswith(('_open', '_mid', '_close')):
        ...
        suffix = tag.split('_')[-1]  # e.g., singleton or test
    return f"{subfamily}_{base}_{suffix}"
#make internal_id
merged_table['internal_id'] = merged_table['tag'].apply(lambda tag: convert_tag_to_internal_id(tag, subfamily))
merged_table
# %%
internal_id_table = merged_table[['tag','internal_id']]
internal_id_table = internal_id_table.rename_axis('rmsk_index').reset_index()
# %%
internal_id_table.to_csv('/home/pc575/rds/rds-kzfps-XrHDlpCeVDg/users/pakkanan/dev/teatime/debug/annotation_mending_revise/workflow_test/THE1C.internal_id.txt', sep='\t')
# %%
