...
#%%
for block_id in partial_frag_id:
    
    table_by_id = repeatmasker_table[(repeatmasker_table.id == block_id) & (repeatmasker_table.repName == subfamily)].copy()
    ...
    index_list=table_by_id.index.to_list()
    ...
    for idx, row in table_by_id.iterrows():
        if row.tag == 'POSTHOC':
            if row.next_tag == 'OPEN':
                ...
                #print(7)
                table_by_id.at[idx,'tag'] = 'OPEN'
        #print(f'block{idx}\t{row.genoName}:{row.genoStart}-{row.genoEnd}\t{row.repStart}-{row.repEnd}:{row.repLeft}\t{row.tag}')
    table_by_id['new_tag'] = None
    #1 look for rows that align nicely first
    indexlist=table_by_id.index.to_list()
    for idx, row in table_by_id.iterrows():
        if row.new_tag is None:
            #print(f'check for potential complete sequnce that somehow avoid the filter: repLeft {row.repLeft}: repLength {row.repLength}>= commonlength {common_length}?')
            if row.repLeft == 0 and row.repLength >= common_length:
                print(0)
                #print(f'COMPLETE sequnce repLeft == 0 and >= {common_length}')
                table_by_id.at[idx,'new_tag'] = 'COMPLETE'
            elif idx == index_list[-1]:
                print(1)
                #print(f'not complete but is the last row CLOSE')
                table_by_id.at[idx,'new_tag'] = 'CLOSE'
            elif (row.next_repStart > row.repEnd) and (row.repLeft >= row.next_repLength):
                print(2)
                table_by_id.at[idx,'new_tag'] = 'OPEN'
            elif (row.repEnd - row.next_repStart < 0.05*common_length) and (row.repStart < row.next_repStart):
                print(f'3: {row.next_repStart} - {row.repEnd}')
                table_by_id.at[idx,'new_tag'] = 'OPEN'
            else:
                table_by_id.at[idx,'new_tag'] = 'CLOSE-check'
    for idx, row in table_by_id.iterrows():
        print(f'{block_id}\t{row.genoName}:{row.genoStart}-{row.genoEnd}\t{row.repStart}-{row.repEnd}:{row.repLeft}\t{row.tag}\t{row.new_tag}')
...
#%%
                ...
                #print(f'this is also not making any sense kind of desperate attemp')
                #print(f'fragments are not next to each other/possible overlap?')
                if row.next_repStart - row.repEnd >= 0.05*common_length:
                    #print(f'the current repEnd {row.repEnd}+next fragment length {row.next_repLength} >= {common_length}  CLOSE')
                    print(4)
                    table_by_id.at[idx,'new_tag'] = 'CLOSE'
                else:
                    #print(f'the current repEnd {row.repEnd}+next fragment length {row.next_repLength} < {common_length}  POSTHOC')
                    print(5)
                    table_by_id.at[idx,'tanew_tagg'] = 'POSTHOC'
    table_by_id['next_tag'] = table_by_id['new_tag'].shift(-1)
    for idx, row in table_by_id.iterrows():
        if row.tag == 'POSTHOC':
            if row.next_tag == 'OPEN':
                print(6)
