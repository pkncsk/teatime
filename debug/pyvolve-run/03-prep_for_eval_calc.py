...
for idx, seed_n in enumerate(seed_list):
    ...
    prealign_filepath = f"{work_subfolder}/simTHE1C_{idx}.prealign.fasta"
    aligned_filepath = f'{work_subfolder}/simTHE1C_{idx}.aligned'
    sequence_alignment.mafft_align(
        input_filepath=prealign_filepath,
        nthread=6,
        output_filepath=aligned_filepath
    )
    chr_table = {
            "hg38": "chr1"
        }
    fix_alignment(aligned_filepath, f'{work_subfolder}/simTHE1C.maf', chr_table)
    # make mafindex
    # Initialize
    chrom = 'chr1'
    maf_filepath = f'{work_subfolder}/simTHE1C.maf'
    mafindex_filepath = f'{work_subfolder}/simTHE1C.mafindex'
    target_species = 'hg38'
