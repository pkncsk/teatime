...
def get_maf_filepath(maf_dir, chrom):
    ...
    return maf_filepath
#%%
def e_val_calc(internal_id, target_species = 'hg38'):
    ...
    try:
        maf_filepath = f'{maf_dir}/simTHE1C.maf'
...
def e_val_calc_batch(subfamily,
    ...
    input_filepath = internal_id_dir
    ...
    output_filepath = f'{e_table_dir}/simTHE1C.e_table.txt'
...
#%%
def main(work_subfolder, idx):
    #initialize 
    global repeatmasker_table, divergence_table,target_species, maf_file_prefix, maf_dir
    maf_dir = work_subfolder
    internal_id_filepath = f'{work_subfolder}/simTHE1C_{idx}.internal_id.txt'
    e_table_filepath = work_subfolder
    repeatmasker_filepath = f'{work_subfolder}/simTHE1C_{idx}.ref.txt'
    repeatmasker_table = pd.read_csv(repeatmasker_filepath, sep='\t')
    divergence_table=divergence_table_prep(divergence_table_filepath)
    #for subfam in ['THE1C']:
    for subfam in subfamily_list:
        e_val_calc_batch(subfam, internal_id_filepath, e_table_filepath)
#%%
